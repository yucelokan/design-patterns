...
struct TestDeveloperJobSeeker: JobContactProtocol{
    var contact: Contact
    ...
    func sendRequestResumeEmail()->String{
        return "Dear \(contact.name), TEST TEST \(contact.desiredSal) in half, I should be able to place you as a contract to hire. Send me your resume!"
    }
}
...
struct JobContacterFactory{
    static func getJobSeeker(contact: Contact)->JobContactProtocol {
        ...
        case .Test:
            return TestDeveloperJobSeeker(contact: contact)
        }
...
contacts.append(Contact(name: "L Roger", email: "lroger@example.com", job: .Test, desiredSal: 165000))
